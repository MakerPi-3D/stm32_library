...
{
  NAND_BANK3_CMD = NAND_FEATURE; //发送设置特性命令
  NAND_BANK3_ADDR = 0X01; //地址为0X01,设置mode
  NAND_BANK3_8_BYTE = mode;      //P1参数,设置mode
  NAND_BANK3_8_BYTE = 0;
  NAND_BANK3_8_BYTE = 0;
  NAND_BANK3_8_BYTE = 0;
...
u32 NAND_ReadID(void)
{
  u8 deviceid[5];
  u32 id;
  NAND_BANK3_CMD = NAND_READID; //发送读取ID命令
...
{
  vu8 data = 0;
  NAND_BANK3_CMD = NAND_READSTA; //发送读状态命令
  data++;
  data++;
  data++;
  data++;
...
u8 NAND_Reset(void)
{
  NAND_BANK3_CMD = NAND_RESET; //复位NAND
...
u8 NAND_ReadPage(u32 PageNum, u16 ColNum, u8 *pBuffer, u16 NumByteToRead)
{
  ...
  else
  {
    ...
    NAND_BANK3_CMD = 0X05;   //随机读指令
    //发送地址
    NAND_BANK3_ADDR = (u8)i;
    NAND_BANK3_ADDR = (u8)(i >> 8);
    NAND_BANK3_CMD = 0XE0;   //开始读数据
    NAND_Delay(30);//等待tADL
...
u8 NAND_CopyPageWithoutWrite(u32 Source_PageNum, u32 Dest_PageNum)
{
  ...
  NAND_BANK3_CMD = NAND_MOVEDATA_CMD0; //发送命令0X00
  //发送源页地址
  ...
  NAND_BANK3_CMD = NAND_MOVEDATA_CMD1; //发送命令0X35
  //下面两行代码是等待R/B引脚变为低电平，其实主要起延时作用的，等待NAND操作R/B引脚。因为我们是通过
  //将STM32的NWAIT引脚(NAND的R/B引脚)配置为普通IO，代码中通过读取NWAIT引脚的电平来判断NAND是否准备
  //就绪的。这个也就是模拟的方法，所以在速度很快的时候有可能NAND还没来得及操作R/B引脚来表示NAND的忙
  //闲状态，结果我们就读取了R/B引脚,这个时候肯定会出错的，事实上确实是会出错!大家也可以将下面两行
  ...
  NAND_BANK3_CMD = NAND_MOVEDATA_CMD2; //发送命令0X85
  //发送目的页地址
  NAND_BANK3_ADDR = (u8)0;
  NAND_BANK3_ADDR = (u8)0;
  ...
  NAND_BANK3_CMD = NAND_MOVEDATA_CMD3; //发送命令0X10
